...
def main(argument_parser, process_genesis_func):
    ...
    # update howell1n2kzv00yjanjpjplqtwucug45lurr8tzgrvj2p => howell1pnnruerze80znqdcl5ka6y3ntpzyu2e4j4yeev
    # update howell1f0gxt604sn3py38u4pecf2ha3x66640cvndpv7 => howell1pln09kv8s7k3ssw0p6ymdwjn7aya3h6mj5mp2x
    raw_genesis = json.dumps(genesis, indent=4, sort_keys=True)
    raw_genesis = raw_genesis.replace(
        'howell1n2kzv00yjanjpjplqtwucug45lurr8tzgrvj2p', 
        'howell1pnnruerze80znqdcl5ka6y3ntpzyu2e4j4yeev')
    raw_genesis = raw_genesis.replace(
        'howell1f0gxt604sn3py38u4pecf2ha3x66640cvndpv7',
        'howell1pln09kv8s7k3ssw0p6ymdwjn7aya3h6mj5mp2x')
...
def process_raw_genesis(genesis, parsed_args):
    ...
    for acc in genesis['app_state']['accounts']:
        ...
        if acc['address'] == 'howell1fs7mmpducjf25j70sk3sz6k5phz2fllmyr5gwz':
            update_vesting_schedule(newAcc)
        if acc['address'] == 'howell1dp0taj85ruc299rkdvzp4z5pfg6z6swaed74e6':
            # (Foundation) sub 978,260 LUNA to give new vesting account
            for coin in newAcc['coins']:
              if coin['denom'] == 'usheer':
                coin['amount'] = str(int(coin['amount']) - 978260000000)
        if acc['address'] == 'howell1nl2vrxr0qzzy4pd9m2mw0q0tvwcxe2mg8shaad':
            update_seed_to_private_vesting_schedule(newAcc)
        if acc['address'] == 'howell10y5usrnwk2ltddm5kenhznl5uj6w3yfga5al4a':
    ...
    newAccounts.append(
        create_module_account(
            'fee_collector', 
            'howell17xpfvakm2amg962yls6f84z3kell8c5lkaeqfa', 
    ...
    newAccounts.append(
        create_module_account(
            'gov', 
            'howell10d07y265gmmuvt4z0w9aw880jnsr700juxf95n', 
            [], 
            ['burner']
        )
    )
    ...
    newAccounts.append(
        create_module_account(
            'distribution', 
            'howell1jv65s3grqf6v6jl3dp4t6c9t9rk99cd8pm7utl', 
            communityPoolCoins, 
            ['basic']
        )
    )
    ...
    newAccounts.append(
        create_module_account(
            'bonded_tokens_pool', 
            'howell1fl48vsnmsdzcv85q5d2q4z5ajdha8yu3nln0mh', 
            [{'amount': str(bondedAmt), 'denom': 'usheer'}], 
            ['burner', 'staking']
        )
    )
    ...
    newAccounts.append(
        create_module_account(
            ...
            'howell1tygms3xhhs3yv487phx3dw4a95jn7t7l8l07dr', 
            [{'amount': str(notBondedAmt), 'denom': 'usheer'}], 
            ['burner', 'staking']
        )
    )
    ...
    newAccounts.append(
        create_module_account(
            'oracle', 
            'howell1jgp27m8fykex4e4jtt0l7ze8q528ux2lh4zh0f', 
            [], 
            ['basic']
        )
    )
    ...
    newAccounts.append(
        create_module_account(
            'market', 
            'howell1untf85jwv3kt0puyyc39myxjvplagr3wstgs5s', 
            [], 
            ['minter', 'burner']
        )
    )
    ...
    newAccounts.append(
        create_module_account(
            'treasury', 
            'howell1vmafl8f3s6uuzwnxkqz0eza47v6ecn0t0yeca7', 
            [], 
            ['minter']
        )
    )
...
def update_vesting_schedule(account):
    ...
    howell_vesting_schedule = {
        'denom': 'usdr',
        'schedules': howell_schedules
    }

    vesting_schedules.append(luna_vesting_schedule)
    vesting_schedules.append(howell_vesting_schedule)
...
def update_seed_to_private_vesting_schedule(account):
    ...
    # howell vesting has no need to be updated
    howell_vesting_schedule = {
        'denom': 'usdr'
    }

    # Find origin howell vesting schedule and use it to new vesting schedule
    for vesting_schedule in account['vesting_schedules']:
        if vesting_schedule['denom'] == 'usdr':
            howell_vesting_schedule['schedules'] = vesting_schedule['schedules']
            break
